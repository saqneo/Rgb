    ...
    public class HidDevice : IHidDevice
    {
        ...
        public bool ReadFeatureData(out byte[] data, byte reportId = 0)
        {
            ...
            try
            {
                ...
                if (success)
                {
                    Buffer.BlockCopy(buffer, 0, data, 0, Math.Min(data.Length, _deviceCapabilities.FeatureReportByteLength));
                }
            }
            catch (Exception exception)
            {
        ...
        public bool WriteFeatureData(byte[] data)
        {
            ...
            Buffer.BlockCopy(data, 0, buffer, 0, Math.Min(data.Length, _deviceCapabilities.FeatureReportByteLength));
        ...
        private bool WriteData(byte[] data, int timeout)
        {
            ...
            Buffer.BlockCopy(data, 0, buffer, 0, Math.Min(data.Length, _deviceCapabilities.OutputReportByteLength));
